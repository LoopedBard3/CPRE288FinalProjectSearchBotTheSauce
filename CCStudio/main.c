...
char left_wheel_speed = 0;  //Holder for the left wheels speed
char right_wheel_speed = 0; //Holder for the right wheel speed
...
void scan()
{
    ...
    oi_setWheels((int) right_wheel_speed * 4, (int) left_wheel_speed * 4);  //Resumes bot movement
...
int main(void)  //Wifi Settings: Raw, port 288, ip 192.168.1.1,
{
    ...
    while (1)   //While forever, or until shutoff
    {
        if(active){ //If we are active
            ...
            if(character == 'G'){   //Activate the robot movement
                active = 1;
            }   
            if(character == 'P'){   //Still allow the robot to play songs
                oi_play_song(MARIO_VICTORY);
            }
        }
